...
const start = async () => {
  ...
  app.post("/createreservation", async (req, res, next) => {
    try {
      console.log("here=> createreservation", req.body);
      const { period, parking_spots_id, user_id } = req.body;
      const reservationId = await controller_Reservation.createReservation({
        period,
        parking_spots_id,
        user_id
      });
      const reservationTicket = await controller_Reservation.getReservationById(
        reservationId
      );
      console.log(reservationTicket);
      if (reservationTicket) {
        res.json({ done: true, result: reservationTicket });
      }
    } catch (err) {
      next(err);
    }
