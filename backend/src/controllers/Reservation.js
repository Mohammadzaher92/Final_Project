...
const initialize_Reservation = async () => {
  ...
  const getReservationById = async id => {
    try {
      const stmtReservation = `SELECT *  FROM Reservation where  reservation_id = ${id}`;
      const stmtParking = `SELECT *  FROM ParkingSpots where  parking_spots_id = ?`;

      const reservation = await db.get(stmtReservation);
      if (!reservation) {
        throw new Error(` reservation with id = ${id} doesnt exist`);
      } else {
        const parkingSpot = await db.get(stmtParking, [
      ...
      throw new Error(
        `could not get the reservation with id = ${id}` + err.message
      );
    }
  };
  const deleteReservation = async id => {
    try {
      ...
      if (result.stmt.changes === 0) {
        throw new Error(
          `could not delete reservation with id = ${id} or wrong id`
        );
      }
      return true;
    } catch (err) {
